...
interface SearchableSelectProps {
  ...
  wide?: boolean
  /** Full width trigger (mobile drawer filters) */
  fullWidth?: boolean
}
...
export function SearchableSelect({
  ...
  return (
    <Popover.Root open={open} onOpenChange={setOpen}>
      <Popover.Trigger
        className={`${triggerBaseClass} ${fullWidth ? 'w-full max-w-none' : wide ? triggerWidthClass.wide : triggerWidthClass.default} ${className}`}
      ...
      <Popover.Portal>
        <Popover.Content
          className="z-[3000] w-[var(--radix-popover-trigger-width)] min-w-[min(var(--radix-popover-trigger-width),calc(100vw-2rem))] max-w-[min(calc(100vw-2rem),20rem)] max-h-[min(320px,50dvh)] overflow-hidden rounded-lg border border-gray-200 bg-white py-1 shadow-lg"
          align="start"
          sideOffset={4}
