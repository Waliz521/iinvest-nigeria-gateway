import { INVEST_INTRO, INVEST_MARKET_CARD } from '../../content/investPageContent'

function FeatureMiniIcon({ label, accent }: { label: string; accent: 'sky' | 'orange' }) {
    ...
    <div className={`flex flex-1 flex-col items-center rounded-2xl px-4 py-4 ${bg}`}>
      <span className="mb-3 flex h-8 w-8 items-center justify-center" aria-hidden>
        {label === 'Growth' ? (
          ...
          <svg className="h-8 w-8" viewBox="0 0 32 32" fill="none">
            <circle cx="16" cy="10" r="4" stroke={color} strokeWidth="2.67" />
            <circle cx="8" cy="22" r="5" stroke={color} strokeWidth="2.67" />
            <circle cx="24" cy="22" r="5" stroke={color} strokeWidth="2.67" />
          </svg>
        ) : (
          <svg className="h-8 w-8" viewBox="0 0 32 32" fill="none">
            <rect x="6" y="6" width="20" height="20" rx="2" stroke={color} strokeWidth="2.67" />
            <path d="M12 20V14M16 20V10M20 20V16" stroke={color} strokeWidth="2.67" strokeLinecap="round" />
          </svg>
        )}
      </span>
...
function MarketGrowthCard() {
  return (
    <div className="relative">
      <div className="rounded-3xl border border-gray-100 bg-white p-8 shadow-xl">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h3 className="text-xl font-bold leading-7 text-slate-900">{INVEST_MARKET_CARD.title}</h3>
          <span className="rounded-full bg-green-100 px-3 py-1 text-sm leading-5 text-green-700">
            {INVEST_MARKET_CARD.badge}
          </span>
        </div>
        ...
        <div
          ...
          aria-hidden
        >
          {INVEST_MARKET_CARD.bars.map((bar) => (
            <div key={bar.label} className="flex flex-1 flex-col items-center gap-2">
              <div
                className={`w-full max-w-[10rem] rounded-t-[10px] bg-gradient-to-t ${bar.gradient} ${bar.heightClass}`}
              />
        ...
        <div className="mt-6 grid grid-cols-3 gap-3">
          {INVEST_MARKET_CARD.features.map((feature) => (
            <FeatureMiniIcon key={feature.label} label={feature.label} accent={feature.accent} />
          ))}
        </div>
      </div>
      <div
...
export function InvestIntroductionSection() {
  return (
      ...
      <div className="mx-auto grid max-w-6xl items-center gap-10 lg:grid-cols-2 lg:gap-16">
        ...
        <div className="order-1 flex flex-col gap-6 lg:order-2">
          ...
          <div className="flex flex-col gap-4">
            {INVEST_INTRO.paragraphs.map((paragraph) => (
              <p key={paragraph.slice(0, 48)} className="text-lg leading-7 text-gray-600">
                {paragraph}
              </p>
            ))}
            <p className="text-lg font-semibold leading-7 text-slate-900">{INVEST_INTRO.closing}</p>
          </div>
        </div>
      </div>
    </section>
