import { INVEST_VALUE_PROPOSITIONS } from '../../content/investPageContent'
import { InvestIcon } from './InvestIcons'

export function InvestValuePropositions() {
  return (
    <section className="bg-white px-4 pt-12 sm:px-6 sm:pt-16 lg:px-8">
      <div className="mx-auto max-w-6xl">
        <ul className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
          {INVEST_VALUE_PROPOSITIONS.map((item) => {
            ...
            return (
              ...
              >
                <span className={`inline-flex h-14 w-14 items-center justify-center rounded-2xl ${iconBg}`}>
                  <InvestIcon name={item.icon} className="h-7 w-7" strokeWidth={2} />
                </span>
