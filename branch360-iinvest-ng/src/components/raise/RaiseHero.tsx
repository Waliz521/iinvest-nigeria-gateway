import {
  ...
  RAISE_HERO_FLOATING,
  RAISE_HERO_TRUST,
} from '../../content/raisePageContent'
import { MY_ACCOUNT_URL } from '../../constants/site'
import { RaiseIcon } from './RaiseIcons'

const HERO_IMAGES = [
  {
    src: 'https://images.unsplash.com/photo-1556761175-5973dc0f32e7?auto=format&fit=crop&w=640&q=80',
  ...
  },
  {
    ...
    alt: 'Entrepreneur reviewing growth metrics',
    className: 'h-64',
...
function HeroBadgeIcon() {
  return (
    <svg className="h-4 w-4 shrink-0 text-white" viewBox="0 0 16 16" fill="none" aria-hidden>
      <path
        d="M2 6h10M2 6v5h10V6M11 6V4H5v2"
        stroke="currentColor"
        strokeWidth="1.33"
        strokeLinecap="round"
    ...
    <div className="relative mx-auto w-full max-w-[700px] lg:mx-0">
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-2xl font-bold leading-8 text-[#00487b]">{RAISE_HERO_FLOATING.capitalRaised.value}</p>
            <p className="text-xs font-medium leading-4 text-gray-600">{RAISE_HERO_FLOATING.capitalRaised.label}</p>
          </div>
        </div>
      </div>
      ...
      <div
        ...
        aria-hidden
      >
        <p className="text-center text-3xl font-bold leading-9 text-slate-900">{RAISE_HERO_FLOATING.successStories.value}</p>
        <p className="text-center text-xs font-medium leading-4 text-gray-600">
          {RAISE_HERO_FLOATING.successStories.label}
        </p>
      </div>
        ...
        <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-10 xl:gap-16">
          <div className="flex flex-col gap-8">
            <div className="inline-flex w-fit max-w-full items-center gap-3 rounded-full border border-white/20 bg-white/20 px-5 py-2.5 outline outline-1 outline-offset-[-1px] outline-white/20">
              <HeroBadgeIcon />
              <span className="text-sm font-semibold leading-5 text-white">{RAISE_HERO.badge}</span>
            </div>

            <h1 className="text-4xl font-bold leading-tight text-white sm:text-5xl sm:leading-[1.1] lg:text-7xl lg:leading-[1.1]">
              <span className="block">{RAISE_HERO.titleLine1}</span>
              <span className="block text-white/95">{RAISE_HERO.titleLine2}</span>
            </h1>

            <p className="max-w-xl text-xl font-normal leading-10 text-white/95 sm:text-2xl">{RAISE_HERO.paragraph}</p>
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              >
                {RAISE_HERO.primaryCta}
                <ArrowIcon className="h-6 w-6" />
              </a>
              <a
                href={RAISE_HERO.secondaryCtaHref}
