import { RAISE_PARTNERS } from '../../content/raisePageContent'
...
export function RaisePartnersSection() {
  return (
    <section id="partners" className="bg-slate-50 px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
      <div className="mx-auto max-w-6xl">
        <div className="text-center">
          <h2 className="text-3xl font-bold leading-10 text-slate-900 sm:text-4xl">{RAISE_PARTNERS.title}</h2>
          <p className="mt-4 text-base leading-6 text-gray-600">{RAISE_PARTNERS.subtitle}</p>
        </div>

        <ul className="mt-12 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
          {RAISE_PARTNERS.partners.map((partner) => (
            <li key={partner.name}>
        ...
        <div className="mt-12 flex flex-wrap items-center justify-center gap-4 border-t border-gray-300 pt-8">
          {RAISE_PARTNERS.badges.map((badge) => (
            <span
              key={badge}
